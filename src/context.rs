use crate::{ErrorLogger, Result, model, shader};

use crate::model::{Model, VertexInput};
use glam::{Mat4, vec3};
use std::sync::Arc;
use std::time;
...
impl RendererContext {
    pub fn new_with(event_loop: &ActiveEventLoop, window: Arc<Window>) -> Result<Self> {
        ...
        let models = model::Model::load("./resource/models/mi35.glb", memory_allocator.clone())?;
    ...
    pub fn update(&mut self, window: &Arc<Window>) -> Result<()> {
        ...
        self.world = Mat4::from_rotation_y(elapsed);
    ...
    fn get_pipeline(
        ...
        let vertex_input_state = VertexInput::per_vertex().definition(&vs).log()?;
